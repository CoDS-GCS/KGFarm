...
from sklearn.metrics import classification_report, make_scorer, confusion_matrix, precision_score, recall_score, \
...
class Recommender:
    ...
    def generate_modeling_data(self):
        ...
        for profile in get_profiles():
            ...
            if missing_value == 0 and (dtype != 'named_entity' or dtype != 'natural_language_text' or dtype != 'string'):
                continue
            else:
                ...
                table_key = generate_column_id(profile['path'], profile['column_name'])
        ...
        self.modeling_data.set_index('Column', inplace=True)
    ...
    def save(self, scores: str, export: bool = True):
        ...
        if export:
            if not os.path.exists('out'):
                os.mkdir('out')
            joblib.dump(self.classifier, 'out/cleaning_{}.pkl'.format('string'), compress=9)
