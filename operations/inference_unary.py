...
from .calculate_embeddings_kglids import EmbeddingCreator
...
def get_column_emb(dataset_name_script,df_train):
    ...
    file1 = pd.read_csv('operations/storage/'+dataset_name_script+'_gnn_Column/mapping/Column_entidx2name.csv')
        ...
        if loadTrainedModel == 1:
            with torch.no_grad():
                ...
                api.download_registry_model("nikimonjazeb", "unary-trans", version=None,
                                            output_path="operations/Models/", expand=True,
                                            stage=None)

                model.load_state_dict(torch.load("operations/Models/kgfarm_gnn_GA_0_DBLP_conf_GSAINT_QM_e10_r3_s10_lr005_layer1_w4_hc32_new_emb_v2_82split_3target.model"))
                out = model.inference(x_dict, edge_index_dict, key2int, new_emb_dict=emb_dict)
                out = out[key2int[subject_node]]
                sorted_abs_indices = np.argsort(np.abs(out))
                flipped_tensor = torch.flip(sorted_abs_indices, [1])
