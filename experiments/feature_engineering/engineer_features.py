...
class EngineerFeatures:
    ...
    def __compute_information_gain(self, train_set: pd.DataFrame, test_set: pd.DataFrame, target: str):
        ...
        self.information_gain = {k: v for k, v in sorted(self.information_gain.items(), key=lambda item: item[1], reverse=True)}
    ...
    def __compute_feature_correlation(self, train_set: pd.DataFrame, test_set: pd.DataFrame, target: str):
        ...
        features_filtered_by_correlation = [feature for feature in features.columns if feature not in features_to_discard]
    ...
    def run(self):
        for _, dataset_info in tqdm(self.experiment_datasets_info.to_dict('index').items(), desc='Datasets processed'):
            ...
            scores = list()
            for train_index, test_index in StratifiedKFold(n_splits=5, shuffle=True, random_state=1).split(df, df[target]):
...
if __name__ == '__main__':
    experiment = EngineerFeatures(path='/Users/shubhamvashisth/Documents/projects/AutoLearn', theta1=0.00, theta2=0.75)
    experiment.run()
