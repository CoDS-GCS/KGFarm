...
class API:
    ...
    def predict_entities_and_feature_views(self, ttl: int = 1000, thresh=0.70, show_query=False):
        ...
        data = self.rdfClient.predict_entities_and_feature_views(thresh, show_query=show_query)
    ...
    def search_tables_on(self, conditions: list, show_query: bool = False) -> pd.DataFrame:
        ...
        df = pd.DataFrame(list(data), columns=['table_name', 'dataset_name', 'origin', 'number_of_columns',
                                               'number_of_rows', 'path']).sort_values('number_of_rows', ignore_index=True, ascending=False)
        return df
